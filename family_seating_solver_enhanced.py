...
Enhanced to compute and visualize:
- Satisfaction ratios for adjacency and table constraints
- Detailed breakdown by constraint type (Must, Cannot, Strongly Prefer, etc.)
...
# ---------------------------------------------------------------------------
# NEW: Compute detailed satisfaction ratios
# ---------------------------------------------------------------------------
...
def compute_satisfaction_ratios(
    ...
    ratios = {
        "adjacency": {
            "must": {"satisfied": 0, "total": 0, "ratio": 0.0},
            "cannot": {"satisfied": 0, "total": 0, "ratio": 0.0},
            "strongly_prefer": {"satisfied": 0, "total": 0, "ratio": 0.0},
            "strongly_avoid": {"satisfied": 0, "total": 0, "ratio": 0.0},
        },
        "table": {
            "must": {"satisfied": 0, "total": 0, "ratio": 0.0},
            "cannot": {"satisfied": 0, "total": 0, "ratio": 0.0},
...
# ---------------------------------------------------------------------------
# NEW: Create satisfaction ratio visualization
# ---------------------------------------------------------------------------

def plot_satisfaction_ratios(ratios, run_dir, filename="satisfaction_ratios.png"):
    ...
    for pref_type in ["must", "cannot", "strongly_prefer", "strongly_avoid"]:
    ...
    if categories:
        ...
        bars1 = ax.bar(x - width/2, satisfied, width, label='Satisfied', color='#32b896', alpha=0.8)
        ...
        ax.set_title('Adjacency Constraints', fontsize=12, fontweight='bold')
        ax.set_xticks(x)
    ...
    if categories:
        ...
        ax.set_ylabel('Count', fontsize=10, fontweight='bold')
        ax.set_title('Table Constraints', fontsize=12, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(categories, fontsize=9)
        ax.legend(fontsize=9)
        ax.grid(axis='y', alpha=0.3)
    ...
    ax.set_title('Detailed Breakdown', fontsize=12, fontweight='bold', pad=20)
...
def make_adjacency_result_heatmap(
    ...
    ratios, run_dir, filename="adjacency_result.png"
):
    """
    Create triangular adjacency heatmap of strong preferences,
    overlaying an X where a strong preference is NOT satisfied.
    ...
    colors = {
        -2: "#4b0082",  # Cannot
        -1: "#b22222",  # Strongly avoid
        0: "#f0f0f0",   # Neutral
        1: "#228b22",   # Strongly prefer
        2: "#006400",   # Must
    }
    ...
    ax.set_title(
        f"Adjacency Preferences vs Actual Seating\n"
        f"Satisfied: {total_adj_satisfied}/{total_adj_constraints} ({adj_ratio_pct:.1f}%) | "
        f"X marks unsatisfied constraints",
        fontsize=13,
        fontweight='bold',
        pad=15
    )
...
def solve_seating(excel_path: str, max_time_seconds: float = 60.0, base_run_dir: str = "runs"):
    ...
    # 8) NEW: Compute satisfaction ratios
    ...
    plot_satisfaction_ratios(ratios, run_dir)
    make_adjacency_result_heatmap(guests, adj_df, seating_plan, chairs_by_table, round_tables, ratios, run_dir)
